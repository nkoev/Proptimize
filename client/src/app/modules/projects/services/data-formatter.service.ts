...
export class DataFormatterService {
  ...
  formToChartData(
    ...
    if (Object.keys(project).length !== 0 && project.mUpdatedAt) {
      projectManDone =
        project.mDone +
        moment(project.mUpdatedAt.toDate()).businessDiff(moment(this.today)) *
          project.managementHours;
    }
    ...
    if (projectManDone) {
        ...
        if (manDays === -1.5) {
          ...
          if (manDays <= moment(this.today).businessDiff(moment(endDate))) {
            ...
            manAmount =
              Math.round(
                ((projectManDone +
                  moment(this.today).businessDiff(moment(endDate)) * manHours) /
                  manTarget +
                  Number.EPSILON) *
                  100
              ) / 100;
      ...
      manAmount =
        manDays === -1.5
          ? 0
          : manDays <= targetInDays
          ? 1
          : Math.round((targetInDays / manDays + Number.EPSILON) * 100) / 100;
    }
    ...
    if (Object.keys(project).length !== 0) {
      projectHoursDone = project.skills.map((skill) => {
        const daysBeen = moment(skill.updatedAt.toDate()).businessDiff(
          moment(this.today)
        );
    ...
    skillsC.controls.forEach((c) => {
      ...
      if (projectHoursDone && projectHoursDone.length) {
        ...
        if (old) {
          days = sum
            ? target >= old.done
              ? Math.ceil((target - old.done) / sum)
              : Math.floor((target - old.done) / old.daily)
            : target > old.done
            ? -1.5
            : Math.floor((target - old.done) / old.daily);
            ...
            if (days === -1.5) {
              ...
              if (days <= moment(this.today).businessDiff(moment(endDate))) {
                ...
                amount =
                  Math.round(
                    ((old.done +
                      moment(this.today).businessDiff(moment(endDate)) * sum) /
                      target +
                      Number.EPSILON) *
                      100
                  ) / 100;
          ...
          amount =
            days === -1.5
              ...
              : days <= targetInDays
              ? 1
              : Math.round((targetInDays / days + Number.EPSILON) * 100) / 100;
          sumSkillHours.push({
            skill,
            start: this.today,
            days,
    ...
    sumSkillHours.forEach((s) => {
      skills.push({
        ...
        completeness: {
          amount: s.amount,
          fill:
            s.days < moment(this.today).businessDiff(moment(endDate))
              ? false
              : true,
        },
      });
  ...
  projectToChartData(
    ...
    project.skills.map((skill) => {
      ...
      days = daily
        ? target >= sumDone
          ? Math.ceil((target - sumDone) / daily)
          : Math.floor((target - sumDone) / daily)
        : target > sumDone
        ? -1.5
        : Math.floor((target - sumDone) / daily);
        ...
        if (days === -1.5) {
          ...
          if (days <= moment(this.today).businessDiff(moment(endDate))) {
            ...
            amount =
              Math.round(
                ((sumDone +
                  moment(this.today).businessDiff(moment(endDate)) * daily) /
                  target +
                  Number.EPSILON) *
                  100
              ) / 100;
    ...
    sumSkillHours.forEach((s) => {
      skills.push({
        ...
        startDate: s.start,
        endDate:
          s.days !== -1.5 ? moment(this.today).businessAdd(s.days)._d : endDate,
        completeness: {
          amount: s.amount,
          fill:
            s.days < moment(this.today).businessDiff(moment(endDate))
